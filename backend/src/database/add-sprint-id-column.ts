...
async function addSprintIdColumn() {
  ...
  try {
    ...
    if (columnCheck.length > 0) {
      console.log(
        `✅ sprintId column already exists as: ${columnCheck[0].column_name} (${columnCheck[0].data_type})`,
      );
    } else {
      ...
      if (verify.length > 0) {
        console.log(
          `✅ Verified: sprintId column exists as: ${verify[0].column_name} (${verify[0].data_type})`,
        );
      } else {
