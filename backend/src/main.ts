  ...
  try {
    ...
    const timeoutPromise = new Promise((_, reject) => {
      setTimeout(() => {
        reject(
          new Error(
            "NestFactory.create() timed out after 60 seconds. Check database connection and migrations.",
          ),
        );
    ...
    console.log(
      `🔧 Port value: ${port} (type: ${typeof port}, raw: ${portRaw})`,
    );

    if (isNaN(port) || port <= 0 || port > 65535) {
      throw new Error(
        `Invalid port number: ${portRaw}. Must be between 1 and 65535.`,
      );
    }
    ...
    try {
      ...
      const heartbeatInterval = setInterval(() => {
        try {
          ...
          console.log(
            `💓 Server heartbeat - listening: ${isListening}, address: ${JSON.stringify(addr)}, time: ${new Date().toISOString()}`,
          );
      ...
      console.log(
        `✅ Heartbeat will show every 5 seconds to confirm server is running.`,
      );
    } catch (error) {
      ...
      if (error instanceof Error) {
        ...
        console.error(
          "Error details (not Error instance):",
...
process.exit = function (code?: number) {
  ...
  const isMigrationScript =
    scriptPath.includes("run-migrations") ||
  ...
  if (code === 0) {
    console.error(
      `⚠️  Blocking exit with code 0 - server should stay running!`,
    );
